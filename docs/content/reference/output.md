...
describes what users get. Pick a format with `-o`, or let binance choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
binance <command> -o table   # aligned columns for reading
binance <command> -o jsonl   # one JSON object per line, for piping
binance <command> -o json    # a single JSON array
binance <command> -o csv     # spreadsheet friendly
binance <command> -o tsv     # tab-separated
binance <command> -o url     # just the URL column
binance <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
binance <command> --fields id,title,url
```
...
```bash
binance <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
binance <command>            # a table, because this is a terminal
binance <command> | wc -l    # JSONL, because this is a pipe
```
