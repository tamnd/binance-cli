// Package cli assembles the binance command tree from the binance
...
// NewApp assembles the kit application from the binance domain. The
...
//
// To add a command, declare it in binance/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := binance.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(binance.Domain{}).Register(app)
